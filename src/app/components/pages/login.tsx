...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If the account is an admin, redirect to admin panel
      if (result.profile?.role === "admin") {
        toast.success("Đăng nhập thành công! Chuyển đến trang Admin...");
        navigate("/admin");
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#FAF7F0] p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-[#1F1F1F]">Chào mừng trở lại</h1>
          <p className="text-[#6B7280] mt-1">Đăng nhập để tiếp tục</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-[#E5E7EB] p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end">
              <Link to="/forgot-password" className="text-sm text-[#D4AF37] hover:underline">Quên mật khẩu?</Link>
            </div>
          ...
          <div className="my-6 flex items-center gap-3">
            ...
            <span className="text-sm text-[#6B7280]">hoặc</span>
            <div className="flex-1 h-px bg-[#E5E7EB]" />
          </div>

          <button onClick={handleGoogle} className="w-full py-2.5 border border-[#E5E7EB] rounded-lg flex items-center justify-center gap-2 hover:bg-gray-50 transition">
          ...
          <p className="text-center mt-6 text-sm text-[#6B7280]">
            Chưa có tài khoản? <Link to="/signup" className="text-[#D4AF37] font-semibold hover:underline">Đăng ký</Link>
          </p>
        </div>
      </div>
    </div>
