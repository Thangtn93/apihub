...
// Add user to context for userApp
...
// Add user to context and verify admin for adminApp
...
// Mount routers
app.route(PREFIX, publicApp);
app.route(PREFIX, userApp);
app.route(PREFIX, adminApp);
app.route(PREFIX, proxyApp);
...
publicApp.get("/health", (c) => c.json({ status: "ok" }));

publicApp.post("/auth/signup", async (c) => {
  try {
...
publicApp.post("/admin/bootstrap", async (c) => {
  try {
...
userApp.get("/user/profile", async (c) => {
...
userApp.put("/user/profile", async (c) => {
  const user = c.get('user');
...
userApp.post("/purchases", async (c) => {
  const user = c.get('user');
...
userApp.delete("/tokens/:id", async (c) => {
  const user = c.get('user');
  await kv.del(`token:${c.req.param("id")}`);
  return c.json({ success: true });
});

userApp.post("/usage-logs", async (c) => {
  const user = c.get('user');
...
userApp.get("/usage-logs", async (c) => {
  const user = c.get('user');
...
adminApp.post("/admin/set-role", async (c) => {
...
adminApp.post("/models", async (c) => {
  ...
  console.log(`Model created successfully: ${id}`);
  return c.json(m);
});
...
adminApp.post("/models/delete", async (c) => {
  const { id } = await c.req.json();
  if (!id) return c.json({ error: "Missing model id" }, 400);
  console.log(`delete-model: attempting to delete model with ID: ${id}`);
  try {
    const existing = await kv.get(`model:${id}`);
    if (!existing) {
      return c.json({ success: false, error: "Model not found or already deleted", searchedKey: `model:${id}` }, 404);
    }
    await kv.del(`model:${id}`);
    console.log(`delete-model: Successfully deleted model: ${id}`);
...
adminApp.put("/admin/users/:id", async (c) => {
  const id = c.req.param("id");
  const updates = await c.req.json();
...
adminApp.post("/seed", async (c) => {
  return c.json({ success: true, message: "Seed disabled. Use admin sync to import models from 9Router." });
});

adminApp.post("/admin/clear-data", async (c) => {
  try {
    ...
    console.log(`clear-data: deleted ${deleted} records`);
...
adminApp.post("/admin/clear-models", async (c) => {
  try {
...
adminApp.post("/admin/test-supabase-query", async (c) => {
  try {
...
adminApp.get("/admin/router-config", async (c) => {
  return c.json({
    ROUTER_API_BASE,
...
adminApp.post("/sync-models", async (c) => {
  try {
    ...
    if (!res.ok) {
      ...
      return c.json({ error: `Failed to fetch from 9Router: ${res.status} ${errText}`, url: fetchUrl }, 500);
    }
    ...
    for (const m of modelsList) {
      ...
      if (existing) {
        ...
        const model = {
          ...
          provider, ownedBy: m.owned_by || provider, category, description: `${displayName} - provided via 9Router`,
...
// =============================================
// PROXY ROUTES (Custom Auth via Token or API Key)
// =============================================

proxyApp.post("/v1/chat/completions", async (c) => {
  try {
...
proxyApp.post("/v1/:endpoint", async (c) => {
  try {
